...
做多传感器融合，将场地进行建模并转化为点云图，用激光雷达对场地进行扫描，用实际扫描出的点云减去建模的点云（差分）剩余的即为场地上机器人的位置，这种方法在对于有一定遮挡的机器人定位似乎有效（比如只露了一个角，在用没定位到的机器人去猜），采用这种方法可以简化标定，即相机和激光雷达的相对位置是已知的，激光雷达再扫描做差分即可定位
