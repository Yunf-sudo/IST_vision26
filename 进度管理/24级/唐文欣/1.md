...
Sudo apt update&&sudo apt upgrade -y##激活当前环境
Mkdir yolo &&cd yolo  ##创建并激活虚拟环境
Python3 -m venv - -system-site-packages venv  ##创建虚拟环境
Source venv/bin/activate ##激活虚拟环境
##建立虚拟环境的目的是避免python环境发生冲突，还有一层目的是如果自己的虚拟环境发生错误时对外界环境也不会产生影响，可以直接删除重新建立
关了窗口重新激活
Cd yolo  source venv/bin/activate
Pip install ultralytics ncnn##安装运行yolo所需要的ultralytics和·ncnn库
##装上摄像头 
Ls /dev/video* ##查看当前连接的video设备，看到video0或者video1即可
Yolo detect predict model=yolo11n.pt  ##下载运行coco的预训练模型)
...
python yolo_detect.py –model=yolo11n_ncnn_model –source=usb0 --resolution=1280x720##运行检测代码，这个代码只是能看到模型识别的效果，实际运行逻辑代码还是要自己去写
  9月7日
